...
public class ImageReader extends SchemaRegistryService implements RecordReaderFactory {
	...
	static final AllowableValue GENERIC_IMAGE_SCHEMA = new AllowableValue(GENERIC_IMAGE_SCHEMA_NAME,
			"Use Generic image Schema", "The schema will be the default image schema.");
	public static final PropertyDescriptor BUFFER_READER_SIZE = new PropertyDescriptor.Builder()
			...
			.description(
					"The bytes of the byte[] buffer used to read the flowfile InputStream as ByteArrayOutputStream to convert to content field record")
			.required(true).defaultValue("1024").addValidator(StandardValidators.INTEGER_VALIDATOR).build();
	...
	public static final PropertyDescriptor IMAGE_CONTENT_FIELD_NAME = new PropertyDescriptor.Builder()
			...
			.description("The name of the field wher the bytes of the image will be stored as bytes").required(true)
			.defaultValue("content").addValidator(StandardValidators.NON_BLANK_VALIDATOR).build();
