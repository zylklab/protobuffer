...
public class TFRecordUtils {
	...
	public static final PropertyDescriptor ENABLE_DICTIONARY_ENCODING = new PropertyDescriptor.Builder()
			.name("enable-dictionary-encoding").displayName("Enable Dictionary Encoding")
			.description("Specifies whether dictionary encoding should be enabled for the TFRecord writer")
			.allowableValues("true", "false").build();

	public static final PropertyDescriptor ENABLE_VALIDATION = new PropertyDescriptor.Builder()
			.name("enable-validation").displayName("Enable Validation")
			.description("Specifies whether validation should be enabled for the TFRecord writer")
			.allowableValues("true", "false").build();
	 ...
	 */
	public static TFRecordConfig createTensorFlowConfig(final PropertyContext context,
	...
	private static Example convert2Example(Record record, RecordSchema schema, ComponentLog componentLogger)
		...
		for (final RecordField field : schema.getFields()) {
			...
			case TIMESTAMP:
			case BYTE:
				componentLogger.warn(String.format("Unsupporte data type conversion %s at field %s with value %s",
						type.toString(), fieldName, value));
				break;
			...
			case ARRAY:
				...
				if (value instanceof byte[]) {
					...
					_log.info(String.format("Content type byte[] detected the size is %s, and the field name is %s ",
							javaBytesValue.length, fieldName));
					componentLogger.debug(
							String.format("Content type byte[] detected the size is %s, and the field name is %s ",
									javaBytesValue.length, fieldName));
					...
					_log.info(String.format(
							"Content type Object[Byte] detected the size is %s, and the field name is %s ",
							javaBytesValue.length, fieldName));
					componentLogger.debug(String.format(
							"Content type Object[Byte] detected the size is %s, and the field name is %s ",
							javaBytesValue.length, fieldName));
				} else {
					Object[] a = (Object[]) value;
					componentLogger.debug(String.format(
							"Unsupporte data type conversion %s at field %s with value type: (%s) - subtype: (%s)  only Arrays of bytes are valid array types",
							type.toString(), fieldName, value.getClass().getSimpleName(),
							a[0].getClass().getSimpleName()));
			...
			case DOUBLE:
				...
				componentLogger
						.debug(String.format("Content type Double detected the size is %s, and the field name is %s ",
								javaDoubleValue, fieldName));
				_log.info(String.format("Content type Double detected the size is %s, and the field name is %s ",
						javaDoubleValue, fieldName));
				feature = Feature.newBuilder();
			...
			case FLOAT:
				...
				componentLogger
						.debug(String.format("Content type Float detected the size is %s, and the field name is %s ",
								javaFloatValue, fieldName));
				_log.info(String.format("Content type Float detected the size is %s, and the field name is %s ",
						javaFloatValue, fieldName));
				feature = Feature.newBuilder();
			...
			case BIGINT:
				...
				componentLogger
						.debug(String.format("Content type BIGINT detected the size is %s, and the field name is %s ",
								javaBigIntValue, fieldName));
				_log.info(String.format("Content type BIGINT detected the size is %s, and the field name is %s ",
						javaBigIntValue, fieldName));
				feature = Feature.newBuilder();
			...
			case INT:
				...
				componentLogger
						.debug(String.format("Content type INT detected the size is %s, and the field name is %s ",
								javaIntValue, fieldName));
				_log.info(String.format("Content type INT detected the size is %s, and the field name is %s ",
						javaIntValue, fieldName));
				feature = Feature.newBuilder();
			...
			case LONG:
				...
				componentLogger
						.debug(String.format("Content type LONG detected the size is %s, and the field name is %s ",
								javaLongValue, fieldName));
				_log.info(String.format("Content type LONG detected the size is %s, and the field name is %s ",
						javaLongValue, fieldName));
				feature = Feature.newBuilder();
			...
			case SHORT:
				...
				componentLogger
						.debug(String.format("Content type SHORT detected the size is %s, and the field name is %s ",
								javaShortValue, fieldName));
				_log.info(String.format("Content type SHORT detected the size is %s, and the field name is %s ",
						javaShortValue, fieldName));
				feature = Feature.newBuilder();
			...
			case STRING:
				...
				componentLogger
						.debug(String.format("Content type STRING detected the size is %s, and the field name is %s ",
								javaStringValue, fieldName));
				_log.info(String.format("Content type STRING detected the size is %s, and the field name is %s ",
						javaStringValue, fieldName));
				feature = Feature.newBuilder();
